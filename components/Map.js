...
function RealRoutePolyline({ coordinates, color = '#F87171' }) {
  ...
  useEffect(() => {
    const fetchRoute = async () => {
      ...
      try {
        ...
        const waypoints = coordinates.slice(1, -1).map(c => `${c[1]},${c[0]}`).join(';');
  ...
  return <Polyline positions={positions} pathOptions={{ color: color, weight: 5, opacity: 0.8 }} />;
}
  ...
  return (
    <div className="w-full h-full bg-[#0F1115]">
      ...
      >
        ...
        <RealRoutePolyline coordinates={coordinates} color="#F87171" />
